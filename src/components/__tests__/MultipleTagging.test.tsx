...
describe('Multiple Tagging Functionality', () => {
  ...
  describe('Adding Multiple Tags to Same Text', () => {
    it('should show visual indicators for multiple tags in content', async () => {
      const contentWithMultipleTags = `
        <p>This text has <span class="my-tag" 
                                data-tag="first-tag second-tag" 
                                style="background: linear-gradient(45deg, #3B82F625 0%, #3B82F625 50%, #EF444425 50%, #EF444425 100%); border-bottom: 3px solid #3B82F6; border-left: 4px solid #EF4444;" 
      ...
      // Check that it has gradient background styling
    ...
    it('should extract multiple tag IDs from content correctly', async () => {
      const content = `
        <p>First <span class="my-tag" data-tag="tag-a tag-b" style="background: linear-gradient(45deg, #3B82F625 0%, #3B82F625 50%, #EF444425 50%, #EF444425 100%);" title="Tagged: Tag A, Tag B">text with two tags</span> here.</p>
        <p>Second <span class="my-tag" data-tag="tag-c" style="background-color: #10B98125;" title="Tagged: Tag C">text with one tag</span> there.</p>
      `
    ...
    it('should handle multiple tags with proper extraction and display', async () => {
      const contentWithMultipleTags = `
        <p>This has <span class="my-tag" 
                             ...
                             style="background: linear-gradient(45deg, #3B82F625 0%, #3B82F625 33.33%, #EF444425 33.33%, #EF444425 66.66%, #10B98125 66.66%, #10B98125 100%); border-bottom: 3px solid #3B82F6; border-left: 4px solid #10B981;" 
      ...
      const multiTaggedElement = screen.getByTitle('Tagged: Tag priority, Tag urgent, Tag high-level')
      expect(multiTaggedElement).toHaveAttribute('data-tag', 'priority urgent high-level')
      expect(multiTaggedElement).toHaveClass('my-tag')
      
      // Check that it has gradient background styling for multiple tags
      const style = multiTaggedElement.getAttribute('style')
      expect(style).toContain('linear-gradient')
      expect(style).toContain('border-left')
  ...
  describe('Removing Multiple Tags from Same Text', () => {
    ...
    it('should support keyboard shortcuts for tag removal on multi-tagged text', async () => {
      ...
      await waitFor(() => {
        ...
        const hasFewerTags = !dataTags?.includes('first') || 
                           dataTags.split(' ').length < 3
        
        // Check if tag removal dialog appeared
        const selectionDialog = screen.queryByText(/which tag/i) || 
                              screen.queryByText(/select tag to remove/i)
  ...
  describe('Visual Enhancements for Multiple Tags', () => {
    it('should show enhanced visual indicators for multiple tags', async () => {
      ...
      expect(multiTaggedElement.getAttribute('data-tag')).toMatch(/important urgent high-priority/)
    })
    ...
    it('should provide hover effects showing all tag names', async () => {
      ...
      await waitFor(() => {
        ...
        const hasHoverEffect = multiTaggedElement.style.transform || 
